...
# script responsible for building mariner packages
...
# if [ "${ID}" != "fedora" ]; then
# 	die "you must use a mariner linux to run this script"
# fi
...
mariner_ver_prefix=$(echo -n ${tracee_version} | cut -d'-' -f1,2 | sed 's:v::g' | sed 's:-:\.:g')
mariner_ver_suffix=$(echo f${version}.${date}.${tracee_commit})

find ./packaging/ -type f -name tracee.spec -exec cp {} . \;

sed -i "s:VERSION:${mariner_ver_prefix}:g" ./tracee.spec
sed -i "s:RELEASE:${mariner_ver_suffix}:g" ./tracee.spec
